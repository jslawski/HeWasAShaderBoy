Shader "Jared/AreaDissolve" {
	Properties{
		_Color("Color", Color) = (1,1,1,1)
		_MainTex("Albedo (RGB)", 2D) = "white" {}
		_Glossiness("Smoothness", Range(0,1)) = 0.5
		_Metallic("Metallic", Range(0,1)) = 0.0
		[HDR]_Emission("Emission", Color) = (1,1,1,1) //Must be tagged with [HDR] for the HDR effect
	...
	//Set in AreaDissolveController.cs
	...
	//Standard "random function" found in the Book of Shaders
	...
	void surf(Input IN, inout SurfaceOutputStandard o) {
		...
		//saturate function keeps value in a [0,1] spectrum
		half sphere = 1 - saturate((dist - _GLOBALMaskRadius) / _GLOBALMaskSoftness);
		clip(sphere - 0.1);
		//step(float a, float b) returns either 1 (a >=b) or 0 (a <= b) 
